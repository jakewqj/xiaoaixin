...
>
> 状态：**全部完成**（2026-08-05）。阶段 1 / 2 / 3 已完成，阶段 4 随之自然消化掉，阶段 5 四项分四步做完。
> 逐条经过见 §九 执行记录。本文件从此是**存档**，不再是待办。
...
**本节的拟稿在阶段 5 实施时改过两个数，最终值以 `src/render/world-data.ts` 的 `PARALLAX` 为准。**
拟稿留在下面，改动理由见 §九「阶段 5」。

```ts
// ↓ 拟稿（2026-08-03）。underwaterMid 的 0.92 实施时改成了 1.00
const PARALLAX = {
...
```ts
sky 0.15 · farIsland 0.30 · waterSurface 0.60 · underwaterFar 0.75
underwaterMid 1.00 ← 拟稿是 0.92 · seabed 1.00
sparkle 0.70 · dustFar 0.90 · actors 1.00 · foreground 1.08 · dustNear 1.15
```

**能不能吃视差，判据是「这层是按格构图还是均匀铺开」，不是「它有多远」。**
`DECOR_VARIANTS` 每格十几件全挤在左右两角、中间留给小爱心和海草床 —— 这是按一格画面构的图，
一旦比沙底慢 8%，格子边界和画面边界就错开。前景礁反过来是按固定周期均匀撒的，密度处处一样，
所以它敢跑 1.08。**纵向一律 1.0**：换气那一下镜头往上摇，水面线、沙底、天空的裁切边界必须一起动。
...
### 阶段 1 · 背景层进 Canvas（视差先不开）★ 已完成 2026-08-04
...
### 阶段 2 · 角色层进 Canvas + 位移搬出 React ★ 已完成 2026-08-04
...
### 阶段 3（2026-08-03）
HUD 进 canvas。详见 `ROADMAP.md`「渲染重构 · 阶段 0–3 前置」和 `CLAUDE.md` 二十二。

### 阶段 1 + 2 + 阶段 4 的世界那一半（2026-08-04）

**做了**：背景六层和角色层全部搬进 canvas，位移/镜头搬出 React，删掉旧的 DOM 渲染。
新增 `src/render/{easing,world-data,swim,camera,world,types}.ts` + `layers/{background,actors}.ts` + `components/WorldCanvas.tsx`；
删掉 `Scene.tsx` `Pet.tsx` `Npc.tsx` `Seagrass.tsx` `Bubbles.tsx` `usePetSwim.ts`。

**三处偏离原计划，都是主动决定，记在这里**：

1. **没做 `?canvas=1` 双轨，直接切过去。** 双轨要求 `usePetSwim` 继续活着，而它每帧 `setX` ——
   留着它就等于留着「每帧 React 全树重渲染」，本阶段最大的一项收益当场作废。
   二分定位改用时间轴：动手前先把旧渲染的截图存成基线（见下），出问题 `git checkout 605754a` 就能拿回旧版。
2. **顺手做了阶段 4 中「拆掉旧渲染」的世界那一半。** HUD 那一半 2026-08-03 已经拆过，
   两半合起来阶段 4 就只剩「去掉双轨开关」——而双轨本来就没建。
3. **`prefers-reduced-motion` 提前到本阶段实现**（原定阶段 4）。世界层离开 DOM 之后
   `index.css` 的媒体查询管不到它了，不当场实现就是直接的功能倒退。
   现在走 `WorldRenderer` 的运行时开关：粒子停、镜头瞬移、精灵停在第一帧、
   换气泡泡照画但不动（这条例外从 `index.css:289` 继承下来——泡泡是「该换气了」的唯一提示）。

**验收**（无头 Chrome + CDP 实测，脚本在会话临时目录，没进仓库）：

| 项 | 结果 |
|---|---|
| 家海草床画面 vs 旧渲染 | 差异 **0.664%**（861/129600 像素），无位置偏差 |
| 海豚潟湖画面 vs 旧渲染 | 差异 **0.408%**（529/129600 像素），无位置偏差 |
| 游动 6 秒期间 App 重渲染 | **0 次**（重构前约 360 次） |
| 巡游速度 | **90.1 逻辑px/秒**，一格 480px ≈ 5.3 秒（SWIM_SPEED=90） |
| 功能冒烟 18 项（开发构建 / 生产构建各一轮） | 18/18，控制台零报错 |
| 每个热区都有 `aria-label` | 26 个按钮，0 个没名字 |

比对手法：CSS 动画大量用负 delay，用 CDP 的 `Animation.seekAnimations` 把它们 seek 到
`currentTime=0`，就会停在 `|delay|` 那个确定进度上；canvas 那边加一个开发构建限定的
`window.__renderFreeze(0)` 把主循环时钟钉死，两边算出同一相位，才能逐像素比。

剩下那 0.4–0.7% 全部是**光栅化差异**，不是摆位错误：旋转的精灵（bob 的 ±2°、海带摇摆）
CSS 走合成器抗锯齿、canvas 走最近邻；海草床是矢量描边；影子是 120→170 的非整数放大。
逐块扫 dx/dy 都是 (0,0) 最吻合，没有任何一处偏了哪怕 1px。

**比对时逮到的两个真 bug**（光看代码不可能发现，两个都是「CSS 的默认值和直觉不一样」）：

1. **`background-position: bottom` 展开是 `50% 100%`**，横向那个 50% 会把 repeat-x 的平铺
   起点挪到 `(容器宽 - 图宽)/2`，不是 0。水下远景剪影因此整体错了 120px。
2. **`<button>` 会把内容垂直居中**，这是 Blink 对按钮的固有行为，不是哪条 CSS 写的。
   `min-h-14`（56px 触摸下限）把按钮撑得比精灵高时，多出来的那截上下平分 ——
   Dolly（48 高的精灵装在 56 高的按钮里）因此整体高了 4px。修掉之后她的区域是 **0.00% 差异**。

**当时没做的**：阶段 5（视差 / 光照 / 前景遮挡 / 镜头 easing）。`overlay` 那块 canvas 也留到阶段 5 再建 ——
当时建就是一块什么都不画的空图层，白占一层合成。（**已于 2026-08-05 做完，见下。**）

### 阶段 5（2026-08-05）

四项分四步做，每步停下来给用户看过再走下一步（本文件三·阶段 5 的要求）。
**一条游戏机制没碰，`SaveData` 一个字段没加，`src/hooks/` 全程只读** —— §四 四道闸一道没破。

新增 `render/parallax.ts` `render/layers/overlay.ts` `render/layers/particles.ts`
和 7 张 PNG（`fg_reef_a/b/c` `fg_kelp_a/b/c` `light_shaft`，`draw-world.mjs` 程序化生成）；
改 `camera.ts` `swim.ts` `world.ts` `world-data.ts` `layers/background.ts` `WorldCanvas.tsx`。

**① 视差。** 先做了一件事保命：把 6 个因子**全设成 1.0**，和改造前的版本逐像素比 ——
4 个水下镜头位 + 2 个水面镜头位，**整帧 0%**。这证明「把一次 `translate(-camX)` 拆成每层各自 `layer()`」
这个改造本身零改动，后面每一个像素的差异都只可能来自因子值，不来自拆函数时的手误。
然后按因子逐层验：沙底和水色恒 0%，剪影在 camX=480 时位移 120px（= 480×0.25，分毫不差），
浮出水面时天空 9.76% / 远岛 18.45%。

`underwaterMid` 从拟稿的 0.92 改成 1.00，理由见 §2.4。实算第 4 格：`ox = 0.92×1440 = 1325`，
屏幕看到 `[1325,1805]`，第 2 格右角落在屏幕 x 3–115、第 3 格左角落在 97–235 ——
两簇珊瑚叠在画面左半边，右边整片空，构图当场垮掉。

**② overlay 层。** 三处推翻了自己刚写完的做法，都是上屏一看不对才改的：

- **光柱不能放 overlay。** 按本文件 §2.1 放在角色之上，alpha 0.45–1.0，结果整屏白带子、
  连小爱心都被洗白。光是水体的一部分 —— 挪到**背景层最后一道**（照在沙上、压在她后面），
  alpha 压到 0.22–0.5。overlay 于是只剩前景礁。
- **前景礁一屏只能一丛。** 按参考图那样一屏两丛，礁丛会坐在家海草床上（海草种在第一格
  x 6%–94%，几乎铺满整屏）。等三天才长成的东西不能被石头盖住。改成一屏一丛、
  第 0 丛卡在第一格最左边之后，实测 12 棵满档一棵没挡。
- **珊瑚不能靠调大生成器的尺寸参数。** `staghorn` 枝粗上限写死 3px、`brainCoral` 沟回间距写死 3.4px，
  尺寸翻倍之后细节密度跟不上，红珊瑚变成光秃秃一棵树、脑珊瑚变成一个巨大的粉团。
  参考图的前景珊瑚**也不比中景大多少**，是「多、挤、叠」才显得近；撑高度的是海带。

  连带一个只有量了才发现的坑：**画布高 ≠ 内容高**。按 `H=140` 生成的礁丛，内容顶只到第 47 行，
  上屏后礁顶停在 y=191，而小爱心的身体在 y 140–175 —— 她永远不会从礁后面过，遮挡这个信号等于白做。
  现在成图一律 `crop()` 到内容边界，PNG 高度就是它的真实高度，摆位才算得准。实测礁顶 y=121。

**③ 镜头 easing + lookahead。** 前瞻 72px、跟随半衰期 0.14s。
**前瞻按速度给，不按朝向** —— 按朝向的话她停下来镜头还偏着，一转身整块画面横甩 2×72px。
按速度给则停下来自然回中、转身要先经过 0，全程连续。代价是 `Swim` 多存一个 `vx`。

指数平滑跟一个匀速移动的目标有固有落后 `速度 × 半衰期 / ln2` = 18px，和 72px 的前瞻抵消，
净前瞻 54px。**这两个数互相抵消，要调就一起调，别只改一个。**

`freeze()` 顺带调了 `camera.settle()`：时钟一钉死 dt 就恒为 0，不落位的话镜头会停在半路，截图不可复现。
这也让阶段 1–2 用的「FREEZE 设成一个巨大的数」的土办法不再必要。

**④ 浮尘 + 水面光斑。** 远近两层浮尘（0.90 / 1.15）+ 水面光斑（0.70）。
位置是 `(索引, 时刻)` 的**纯函数**，不存粒子数组、不随机生成 —— 时钟钉死可复现、
减弱动态效果时原地不动即可、切后台回来不会一次补上几千帧。环境气泡本来就是这么写的。

全项目第二处用 `fillRect` 而非 `drawImage`（第一处是海草床的矢量描边）：一颗浮尘就是 1–2 个逻辑像素，
出 PNG 只会在整数倍放大时多一道采样。§2.5 那条约束管的是**美术素材**，那些照旧全走 `/assets/` 路径。

调参两次才对：第一版一屏 21 颗近白色点子，像屏幕脏了。**问题在色相不在明度** ——
近白色调低到看不见、调高就刺眼，中间没有可用档位；换成偏水色的浅青之后 alpha 0.4 才两头都成立。
密度同时降到一屏 10 颗。
...
| 项 | 结果 |
|---|---|
| 全因子设 1.0 vs 改造前，6 个镜头位 | 整帧 **0%**（证明拆函数零改动） |
| 剪影层位移 @camX 480 | **120px** = 480×0.25，与因子一致 |
| overlay 最高点 / 横向占比 | y=**121**（她的身体在 140–175）/ 49–181 of 480 px |
| 家海草床 12 棵满档 | **一棵没挡** |
| 镜头：向右 / 向左 / 停住 | 屏幕 x **186.4 / 293.4 / 239.9**，速度仍 **90.0** px/s |
| `round(camX)` 方向反复 | **2 / 1321 帧**（就是掉头那下），无抖动 |
| 粒子单帧像素占用 | 35 / 129600（0.03%）—— 单帧确实看不出，效果全在飘 |
| 浮尘漏到水面线以上 | **0 像素** |
| `prefers-reduced-motion` | 粒子和前景礁照画但一动不动（4 秒后两块 canvas 指纹完全一致）；镜头死钉中线、无前瞻 |
| 冒烟（开发正常 / 开发减弱动态 / 生产） | 三轮，按钮零缺失、控制台零报错 |
| `npm run build` / `npm run lint` | 通过 / **24 problems**，阶段 5 全程一条没加 |

**宪法没动一条。** 二十一·4 原文就写明「允许追加……光照/色调层（水下光柱）、天气/氛围粒子层、
前景遮挡层（角色游到礁石后面）」，§十二 + 二十一·3 已放开镜头 easing/lookahead 且缩放仍禁止。
本阶段全部落在已放宽的范围内，只有本文件 §2.4 的拟稿数值需要更正。

### 下一步

渲染重构全部完成，本文件转为存档。后续回到 `ROADMAP.md` 的 Season 编号（S3 · 红海全开）。
