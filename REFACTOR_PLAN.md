...
>
> 状态：**阶段 1 / 2 / 3 已完成，阶段 4 随之自然消化掉**（见 §九 执行记录）。**只剩阶段 5（视差与光照）。**
...
**没做的**：阶段 5（视差 / 光照 / 前景遮挡 / 镜头 easing）。`overlay` 那块 canvas 也留到阶段 5 再建 ——
现在建就是一块什么都不画的空图层，白占一层合成。

### 下一步

阶段 5。这一阶段才「变好看」，且允许画面变化，所以按原计划**每改一项停下来给你看**，不要一口气全上。
