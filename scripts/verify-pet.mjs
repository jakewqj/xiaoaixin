// Verifies pet sprite sheets: dimensions, per-frame pixel counts, palette.
...
const FW = 192
const sheets = { idle: 4, swim: 6, eating: 4, surfacing: 4, sleeping: 2 }
let ok = true
for (const [name, frames] of Object.entries(sheets)) {
  const { w, h, px } = readPng(`public/assets/pet/pixel/${name}.png`)
  const dimOk = w === FW * frames && h === 64
  if (!dimOk) ok = false
  console.log(`${name}.png ${w}x${h} ${dimOk ? 'OK' : 'BAD dim'}`)
  const counts = []
